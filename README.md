...
Стек: Go `api/` (тонкий шлюз) + Rust `compute/` (детекция, маска, vault) + Redis.
...
| [`hack.md`](hack.md) | Подробный playbook дня: архитектура, алгоритмы детекции/маски, промпты треков, критерии приёмки. |
...
| [`api/AGENTS.md`](api/AGENTS.md) | Правила и структура пакета Go `api/`. |
| [`compute/AGENTS.md`](compute/AGENTS.md) | Карта модулей Rust `compute/` и команды `cargo`. |
| [`compute/CLAUDE.md`](compute/CLAUDE.md) | Shim compute → корневой `AGENTS.md` + пакетный `AGENTS.md`. |
...
| [`.cursor/rules/go.mdc`](.cursor/rules/go.mdc) | Конвенции Go при работе в `api/**/*.go`. |
| [`.cursor/rules/rust.mdc`](.cursor/rules/rust.mdc) | Конвенции Rust при работе в `compute/**/*.rs`. |
